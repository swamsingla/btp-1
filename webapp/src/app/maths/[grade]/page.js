...
import { getGrades, getAreaGroups, getConcept, getEntryPoints, areaColor } from '@/lib/knowledge-graph';

export async function generateStaticParams() {
  return getGrades().map(g => ({ grade: String(g) }));
}
...
export default async function MathsGradePage({ params }) {
  ...
  const areaGroups = getAreaGroups(grade);
  ...
  const entryPoints = getEntryPoints(grade);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {sortedAreas.map(area => {
          ...
          return (
            <a key={area} href={`#${area.replace(/\s+/g, '-').toLowerCase()}`}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {concepts.map(concept => {
                  const prereqNames = concept.prerequisites
                    .map(slug => getConcept(slug)?.canonical_name)
                    .filter(Boolean)
                    .slice(0, 2);
                  ...
                  return (
                    ...
                    >
                      ...
                      <h3 className="font-semibold text-gray-900 dark:text-gray-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors text-sm leading-snug mb-1">
