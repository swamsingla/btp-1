...
{\normalsize \textbf{Project Repository:}\enspace\setlength{\fboxsep}{3pt}\colorbox{lightyellow}{\url{https://github.com/swamsingla/btp-1/}}}
...
similar to the level at which a Wikipedia article would exist.
...
The review was a substantial effort, and the model's suggestions
...
The solution in Phase~2 is Retrieval-Augmented Generation (RAG): assembling a rich context for every concept
...
The third source is externally fetched Wikipedia summaries for additional factual grounding,
...
A global search bar finds concepts across all grades and subjects.
...
While the current platform establishes a robust structural and generative foundation, several natural extensions can be done. 

First, the knowledge graph currently operates as a static navigational tool, requiring self-directed exploration. By integrating diagnostic assessments, the platform could transition toward active adaptive learning. If a student struggles with a concept, a short, dynamically generated quiz could diagnose specific gaps in their prerequisite understanding and automatically plot a personalised learning path through the graph to remediate those exact deficiencies.

Second, the content generation pipeline is presently entirely textual and mathematical. For subjects such as Geometry and Biology, visual representation is inseparable from conceptual understanding. Future work would incorporate multimodal vision models to extract, translate, or computationally recreate textbook diagrams and experimental setups, ensuring that the visual components of the curriculum are just as accessible as the text.

Finally, relying on native generation through Indic language models opens a direct path to audio accessibility. Integrating Text-to-Speech (TTS) capabilities natively for Hindi, Telugu, and Odia would provide spoken playback for all generated content. This multimodal delivery would serve students with visual impairments or lower reading literacy, further reducing the friction between the student and the curriculum.
...
The platform gives a student a map of what they are trying to learn.
