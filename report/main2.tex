...
All language model inference ran on a compute cluster with three GPUs of approximately
10~GB each.
The model used throughout was Meta's LLaMA~3.1~8B Instruct.
...
\medskip
\noindent\textbf{Independently generated pages were inconsistent with each other.}
When the same mathematical idea appeared across different chapters or sections,
the pipeline generated each occurrence in complete isolation.
The resulting pages used different notation and occasionally different terminology
for the same underlying concept, because no generation run had any awareness of any other.
A student reading across chapters encountered subtle contradictions they should never have faced.
